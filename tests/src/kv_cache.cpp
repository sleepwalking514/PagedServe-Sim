...
public:
    PagedKvCache(ModelConfig model, bool prefix_cache_enabled)
        : model_(std::move(model)), prefix_cache_enabled_(prefix_cache_enabled), blocks_(model_.total_blocks) {
    ...
    AdmissionResult admit(RequestRuntime& request) override {
        const std::size_t block_count = ceil_div(request.total_tokens(), model_.block_tokens);
        ...
        if (prefix_cache_enabled_) {
            const std::size_t complete_prompt_blocks = request.prompt_tokens() / model_.block_tokens;
            cached_ids.reserve(complete_prompt_blocks);
            for (std::size_t block_index = 0; block_index < complete_prompt_blocks; ++block_index) {
                const auto begin = request.spec.prompt_tokens.begin() +
                                   static_cast<std::ptrdiff_t>(block_index * model_.block_tokens);
                const auto end = begin + static_cast<std::ptrdiff_t>(model_.block_tokens);
                ...
                const Block& block = blocks_[static_cast<std::size_t>(cache_it->second)];
    ...
    bool append_tokens(RequestRuntime& request, const std::size_t token_count) override {
        for (std::size_t token = 0; token < token_count; ++token) {
            if (request.stored_tokens >= request.block_table.size() * model_.block_tokens) {
                return false;
            }

            const std::size_t block_index = request.stored_tokens / model_.block_tokens;
            int& block_id = request.block_table[block_index];
            if (block_id < 0) {
                if (request.reserved_blocks == 0 || reserved_unmaterialized_ == 0 || free_block_ids_.empty()) {
                    return false;
                }
    ...
    void cache_completed_prompt_block(RequestRuntime& request, const std::size_t block_index,
    ...
    [[nodiscard]] KvCacheStats stats() const override {
        ...
        result.available_blocks = free_block_ids_.size() >= reserved_unmaterialized_
        ...
        for (const Block& block : blocks_) {
            ...
            if (block.request_refs > 0 || block.cache_pinned) {
                result.physical_block_utilization += static_cast<double>(block.used_tokens);
            }
        }
        const double capacity = static_cast<double>(result.allocated_blocks * model_.block_tokens);
        if (capacity > 0.0) {
            result.physical_block_utilization /= capacity;
            result.fragmentation_ratio = 1.0 - result.physical_block_utilization;
        }
...
private:
    ...
    static std::size_t ceil_div(const std::size_t value, const std::size_t divisor) {
        return divisor == 0 ? 0 : (value + divisor - 1) / divisor;
    }

    bool ensure_capacity(const std::size_t required_blocks) {
        while (free_block_ids_.size() < reserved_unmaterialized_ + required_blocks) {
...
public:
    explicit ContiguousKvCache(ModelConfig model) : model_(std::move(model)), blocks_(model_.total_blocks) {}

    AdmissionResult admit(RequestRuntime& request) override {
        const std::size_t needed = ceil_div(request.total_tokens(), model_.block_tokens);
        ...
        for (std::size_t index = 0; index < blocks_.size(); ++index) {
            if (!blocks_[index].owned) {
                ...
                if (run_length == needed) {
                    request.block_table.clear();
                    for (std::size_t block = run_start; block < run_start + needed; ++block) {
    ...
    bool append_tokens(RequestRuntime& request, const std::size_t token_count) override {
        for (std::size_t token = 0; token < token_count; ++token) {
            ...
            const std::size_t block_index = request.stored_tokens / model_.block_tokens;
            Block& block = blocks_[static_cast<std::size_t>(request.block_table[block_index])];
            block.used_tokens = std::max(block.used_tokens, request.stored_tokens % model_.block_tokens + 1);
            ++request.stored_tokens;
        }
        return true;
    }

    void cache_completed_prompt_block(RequestRuntime&, std::size_t, std::uint64_t) override {}
...
std::unique_ptr<KvCache> make_kv_cache(const ModelConfig& model, const ServiceConfig& service) {
...
std::uint64_t hash_prompt_block(const std::span<const std::uint32_t> tokens) {
    // FNV-1a is sufficient for a deterministic simulator key; production code should use a stronger hash.
