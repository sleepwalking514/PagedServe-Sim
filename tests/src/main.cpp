...
template <typename T>
void set_if_present(const std::unordered_map<std::string, std::string>& values, const std::string& key, T& target);

template <>
void set_if_present(const std::unordered_map<std::string, std::string>& values, const std::string& key,
                    std::size_t& target) {
...
template <>
void set_if_present(const std::unordered_map<std::string, std::string>& values, const std::string& key, double& target) {
    if (const auto it = values.find(key); it != values.end()) {
...
void apply_config_file(CliOptions& options) {
    ...
    set_if_present(values, "service.prefill_chunk_tokens", service.prefill_chunk_tokens);
    set_if_present(values, "service.max_active_requests", service.max_active_requests);
    ...
    set_if_present(values, "service.prefix_cache_enabled", service.prefix_cache_enabled);
    set_if_present(values, "workload.request_count", workload.request_count);
    set_if_present(values, "workload.seed", workload.seed);
    set_if_present(values, "workload.arrival_rate_per_second", workload.arrival_rate_per_second);
    set_if_present(values, "workload.shared_prefix_tokens", workload.shared_prefix_tokens);
    if (const auto it = values.find("service.kv_layout"); it != values.end()) {
        service.kv_layout = it->second == "contiguous" ? KvLayout::Contiguous : KvLayout::Paged;
    }
    if (const auto it = values.find("service.scheduler_policy"); it != values.end()) {
        service.scheduler_policy = it->second == "prefill_first" ? SchedulerPolicy::PrefillFirst
...
void parse_cli_overrides(CliOptions& options, const int argc, char** argv) {
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--config") {
            ...
            options.workload.kind = serving::workload_kind_from_string(require_value(index, argc, argv));
        } else if (argument == "--requests") {
            options.workload.request_count = static_cast<std::size_t>(std::stoull(require_value(index, argc, argv)));
        } else if (argument == "--seed") {
            options.workload.seed = static_cast<std::uint32_t>(std::stoul(require_value(index, argc, argv)));
            ...
            std::cout << "Usage: serving_sim [options]\n"
                         ...
                         "  --cuda-validate           Compare the CUDA kernel with a CPU reference\n";
            std::exit(0);
        } else {
...
serving::SimulationConfig scenario_config(const serving::SimulationConfig& base, const std::string& scenario) {
...
void write_comparison_csv(const std::filesystem::path& path, const std::vector<serving::SimulationResult>& results) {
    if (results.empty()) {
        return;
    }
    ...
    output << "scenario,slo_goodput_requests_per_second,throughput_tokens_per_second,ttft_p50_ms,ttft_p95_ms,"
              "tpot_p50_ms,tpot_p95_ms,average_block_utilization,peak_fragmentation_ratio,cache_evictions\n";
    output << std::fixed << std::setprecision(4);
    for (const auto& result : results) {
        output << result.scenario_name << ',' << result.slo_goodput_requests_per_second << ','
               << result.throughput_tokens_per_second << ',' << result.ttft_p50_ms << ',' << result.ttft_p95_ms << ','
               << result.tpot_p50_ms << ',' << result.tpot_p95_ms << ',' << result.average_block_utilization << ','
               << result.peak_fragmentation_ratio << ',' << result.cache_evictions << '\n';
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (!options.cuda_profile_output.empty()) {
            ...
            serving::write_cuda_decode_profile(options.cuda_profile_output, benchmark_config);
            std::cout << "Wrote CUDA decode profile to " << options.cuda_profile_output << '\n';
            return 0;
        }
        if (options.validate_cuda) {
            const serving::CudaValidationResult validation = serving::validate_cuda_paged_attention();
            std::cout << "CUDA validation passed: max_abs_error=" << validation.max_absolute_error
                      << ", mean_abs_error=" << validation.mean_absolute_error << '\n';
            return 0;
        }

        std::filesystem::create_directories(options.output_directory);
        const std::vector<serving::RequestSpec> workload = serving::generate_workload(options.workload);
        serving::write_trace_csv(options.output_directory / "workload.csv", workload);
        ...
        serving::LatencyModel latency_model = options.decode_profile_path.empty()
                                                   ? serving::LatencyModel::analytical()
                                                   : serving::LatencyModel::from_decode_profile(options.decode_profile_path);
        std::vector<serving::SimulationResult> results;
        for (const std::string& scenario : scenarios) {
            const serving::SimulationConfig config = scenario_config(options.simulation, scenario);
            serving::ServingSimulator simulator(config, latency_model);
            serving::SimulationResult result = simulator.run(workload, scenario);
            serving::write_summary_csv(options.output_directory / (scenario + "_summary.csv"), result);
            serving::write_requests_csv(options.output_directory / (scenario + "_requests.csv"), simulator.requests());
            results.push_back(result);

            std::cout << scenario << ": goodput=" << result.slo_goodput_requests_per_second
                      << " req/s, throughput=" << result.throughput_tokens_per_second
                      << " tok/s, TTFT p95=" << result.ttft_p95_ms << " ms, TPOT p95=" << result.tpot_p95_ms
                      << " ms\n";
        }
        if (scenarios.size() > 1) {
            write_comparison_csv(options.output_directory / "comparison.csv", results);
        }
