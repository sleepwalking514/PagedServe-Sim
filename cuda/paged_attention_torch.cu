...
// Keep the extension and the standalone benchmark on the same kernel
// implementation.
#include "paged_attention.cu"

namespace serving {

torch::Tensor torch_paged_attention_decode(torch::Tensor query,
                                           torch::Tensor key_cache,
                                           torch::Tensor value_cache,
                                           torch::Tensor block_table,
                                           torch::Tensor sequence_lengths) {
  TORCH_CHECK(query.is_cuda() && key_cache.is_cuda() && value_cache.is_cuda() &&
                  block_table.is_cuda() && sequence_lengths.is_cuda(),
  ...
  TORCH_CHECK(block_table.scalar_type() == torch::kInt &&
                  sequence_lengths.scalar_type() == torch::kInt,
  ...
  TORCH_CHECK(query.dim() == 3 && key_cache.dim() == 4 &&
              ...
              "expected query [B, Hq, D] and KV caches [num_blocks, Hkv, "
              "block_tokens, D]");
  ...
  TORCH_CHECK(query.size(2) == key_cache.size(3) &&
              ...
              "head dimensions must match and query heads must be divisible by "
              "KV heads");
  TORCH_CHECK(query.size(0) > 0 && query.size(1) > 0 && key_cache.size(0) > 0 &&
                  key_cache.size(1) > 0 && key_cache.size(2) > 0 &&
                  query.size(2) > 0 && query.size(2) <= 128,
              "batch, head counts, block size, and head_dim must be positive; "
              "head_dim must be <= 128");
  TORCH_CHECK(block_table.size(1) > 0,
              "block_table must contain at least one logical block");

  const int min_sequence_length = sequence_lengths.min().item<int>();
  const int max_sequence_length = sequence_lengths.max().item<int>();
  TORCH_CHECK(min_sequence_length > 0, "sequence lengths must be positive");
  TORCH_CHECK(max_sequence_length <= block_table.size(1) * key_cache.size(2),
              "a sequence length exceeds block_table capacity");
  const int min_block_id = block_table.min().item<int>();
  const int max_block_id = block_table.max().item<int>();
  TORCH_CHECK(min_block_id >= 0 && max_block_id < key_cache.size(0),
  ...
  const std::size_t shared_bytes =
      static_cast<std::size_t>(query.size(2)) * sizeof(float);
  cudaStream_t stream =
  ...
  paged_attention_decode_kernel<<<grid, block, shared_bytes, stream>>>(
      ...
      block_table.data_ptr<int>(), sequence_lengths.data_ptr<int>(),
      reinterpret_cast<__half *>(output.data_ptr<at::Half>()),
      static_cast<int>(query.size(1)), static_cast<int>(key_cache.size(1)),
      static_cast<int>(query.size(2)), static_cast<int>(key_cache.size(2)),
