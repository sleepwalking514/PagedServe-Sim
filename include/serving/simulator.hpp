...
public:
  ...
  [[nodiscard]] SimulationResult run(std::vector<RequestSpec> workload,
...
private:
  ...
  void complete_iteration(const std::vector<ScheduledWork> &work,
                          double completion_time_ms);
  [[nodiscard]] std::size_t active_request_count() const;
  [[nodiscard]] std::size_t average_scheduled_decode_context_length(
  ...
  std::vector<std::size_t> actual_output_tokens_;
  double now_ms_ = 0.0;
  ...
  std::size_t processed_prefill_tokens_ = 0;
  double peak_internal_waste_ratio_ = 0.0;
  double peak_external_fragmentation_ratio_ = 0.0;
  std::size_t peak_allocated_blocks_ = 0;
